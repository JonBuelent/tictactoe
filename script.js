...
// von kreuz zu kreis ändern (aktiver Player)
...
function checkForWin() {
    ...
    if (fields[0] == fields[4] && fields[4] == fields[8] && fields[0]){
        winner= fields[0];
        document.getElementById('line-8').style.transform = 'rotate(-135deg) scaleX(1) scaleY(1)';
    }
    //Reihe diagonal rechts oben links unten
    if (fields[2] == fields[4] && fields[4] == fields[6] && fields[2]){
        winner= fields[2];
        document.getElementById('line-7').style.transform = 'rotate(135deg) scaleX(1) scaleY(1)';
    }
    showWinner(winner);
...
//    }
    ...
    function showCrossAsWinner() {
        setTimeout(function () {
            ...
            document.getElementById('game-over-window').innerHTML = `
                <div class="winner-statement"> <img class="img-winner" src="img/cross.png" alt=""> hat gewonnen!</div>
                <button id="restartButton" class="restart-btn" onclick="restart()">Restart</button>`
            }, 1000);
}
    ...
    function showCircleAsWinner() {
        setTimeout(function () {
            document.getElementById('itsOver').classList.remove('d-none');
            document.getElementById('game-over-window').innerHTML = `
                <div class="winner-statement"> <img class="img-winner" src="img/circle.png" alt=""> hat gewonnen!</div>
                <button id="restartButton" class="restart-btn" onclick="restart()">Restart</button>`
            }, 1000);
}
    function drawResult() {
        setTimeout(function () {
            document.getElementById('itsOver').classList.remove('d-none');
            document.getElementById('game-over-window').innerHTML = `  
            <div id="draw" class="winner-statement">Draw!</div>
            <button id="restartButton" class="restart-btn" onclick="restart()">Restart</button>`;
            }, 1000);
}